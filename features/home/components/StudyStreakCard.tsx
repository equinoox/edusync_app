...
export default function StudyStreakCard() {
  ...
  return (
    <section className="edusync-enter edusync-card-motion relative overflow-hidden rounded-2xl border border-white/10 bg-slate-900/80 p-5 shadow-2xl shadow-slate-950/30">
      ...
      <div className="relative flex min-h-44 flex-col items-center justify-center gap-6 lg:flex-row lg:justify-between">
        <div className="flex min-w-0 flex-1 flex-col items-center">
          ...
          <div className="mt-7 grid w-full max-w-3xl grid-cols-7 gap-2 sm:gap-4">
            {days.map((day) => (
              <div key={day.label} className="flex min-w-0 flex-col items-center gap-3">
                <span
                  className={`flex h-11 w-11 items-center justify-center rounded-full border text-white shadow-lg sm:h-12 sm:w-12 ${
                    day.completed
                ...
                >
                  {day.completed && <CheckIcon className="h-6 w-6 stroke-[3]" />}
                </span>
        ...
        <div className="relative flex max-w-xs items-center justify-center gap-4 rounded-2xl border border-white/5 bg-slate-950/35 p-4 text-center sm:text-left">
          <PaperAirplaneIcon className="h-10 w-10 shrink-0 rotate-[-18deg] text-violet-300" />
